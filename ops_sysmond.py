...
MAX_CPU_USAGE_PER_PROCESS = 75
MAX_MEM_USAGE_PER_PROCESS = 75
# Tables definitions
...
def get_process_info(x):
    ...
    cpu_usage = int(y[8].split(".")[0])
    memory_usage = int(y[9].split(".")[0])
    if cpu_usage >= MAX_CPU_USAGE_PER_PROCESS or
        memory_usage >= MAX_MEM_USAGE_PER_PROCESS:
        log_event("SYSMON_WARN",
                ["pid", int[y[0]]],
                ["cpu_usage", cpu_usage],
